...
class Options:
  def __init__(self):
      self.input = '.'
      self.g = 'gene_count_matrix.csv'
      self.t = 'transcript_count_matrix.csv'
      self.length = 75
      self.pattern = '.'
      self.cluster = False
      self.string = "MSTRG"
...
def getGeneID(s, ctg, tid, RE_GENE_ID, RE_GENE_NAME):
...
class DESeqHeteroPlugin:
 ...
 def output(self, outputfile):
  ...
  opts = Options()
  opts.input = self.inputfile
  opts.g = outputfile+"/"+opts.g
  opts.t = outputfile+"/"+opts.t
...
#            f = open(s[1])
        ...
        for i,v in enumerate(split):
            if is_transcript(v):
                ...
                #try:
                g_id=getGeneID(v[8], v[0], t_id, RE_GENE_ID, RE_GENE_NAME)
                #except:
...
##                        transcriptList.append((g_id, t_id, int(ceil(coverage*transcript_len/read_len))))
                ...
                coverage=getCov(v[8], RE_COVERAGE)
                transcript_len=0
            if v[2]=="exon":
